...
public class LogMgr implements Iterable<BasicLogRecord> {
   ...
   private int currentpos; // to find each record 
    ...
    */
   public LogMgr(String logfile) { //imp
      this.logfile = logfile;
      int logsize = SimpleDB.fileMgr().size(logfile); // return current block no = (logfileSize()/ 400) -1
      if (logsize == 0)
         appendNewBlock(); // appending new block to log file 
      else {
         currentblk = new Block(logfile, logsize-1); // Block(filename, block no) 
         mypage.read(currentblk); // reading the current block of log file to log page
    ...
    */
   public synchronized Iterator<BasicLogRecord> iterator() { //imp
      flush();
    ...
    */
   public synchronized int append(Object[] rec) { //imp
   ...
   private void appendVal(Object val) {
      ...
      currentpos += size(val); //update current position
   }
   ...
   private void appendNewBlock() {
      ...
      currentpos = INT_SIZE; //update current position
