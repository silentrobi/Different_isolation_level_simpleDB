...
class LogIterator implements Iterator<BasicLogRecord> {
   private Block blk;
   private Page pg = new Page(); // create additional page 
   private int currentrec; 
   
   /**
    * Creates an iterator for the records in the log file (disk),
   ...
   public BasicLogRecord next() {
      if (currentrec == 0) 
         moveToNextBlock(); // next block of log file in reverse order
   ...
   private void moveToNextBlock() {
      blk = new Block(blk.fileName(), blk.number()-1); // reading from backward
      pg.read(blk); // read block into the log page
      currentrec = pg.getInt(LogMgr.LAST_POS); // last position of the log page is current record position
   }
}
