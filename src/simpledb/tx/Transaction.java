...
public class Transaction {
	...
	private static int isolationLevel = Connection.TRANSACTION_SERIALIZABLE; //default and value is 8
	...
	public int getInt(Block blk, int offset) {
		Buffer buff=null;
		if(isolationLevel == 8) { //Serializable
			concurMgr.sLock(blk);
			buff = myBuffers.getBuffer(blk);
			System.out.println("Tx: "+ txnum +" reads "+ buff.getInt(offset));
			return buff.getInt(offset);
		}else if(isolationLevel == 4){ //Repeatable Reads
			concurMgr.sLock(blk);
			buff = myBuffers.getBuffer(blk);
			System.out.println("Tx: "+ txnum +" reads "+ buff.getInt(offset));
			return buff.getInt(offset);
		}else if(isolationLevel == 2){ // Read Committed
			concurMgr.sLock(blk);
			buff = myBuffers.getBuffer(blk);
			
			System.out.println("Tx: "+ txnum +" reads "+ buff.getInt(offset));
			int value = buff.getInt(offset);
			concurMgr.sLockRelease(blk);
			return value;
		}else if(isolationLevel == 1){ //Read Uncommited
			buff = myBuffers.getBuffer(blk);
			System.out.println("Tx: "+ txnum +" reads "+ buff.getInt(offset));
			return buff.getInt(offset);
		}else{
			throw new  IsolationLevelExceptionError();
		}	
	}
	...
	public void setInt(Block blk, int offset, int val) {
		...
		System.out.println("Tx no: "+txnum + " write " + val+ " to block: "+ blk.number());
	}
	...
	public int size(String filename) {
		...
		}else{ // for other isolation levels
			return SimpleDB.fileMgr().size(filename);
		}
	...
	public static int getIsolationLevel() { // to get Isolation level
		return isolationLevel;
	}

	public static void setIsolationLevel(int isolationLevel) { // to set isolation level
