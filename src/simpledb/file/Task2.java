...
public class Task2 {
   ...
   public synchronized String getString(int offset) {
      contents.position(offset);
      String val ="";
      char token=contents.getChar();
      while (token != '\0'){ //check end of the string
    	  val += token;
          token= contents.getChar();
      }
      return val;
   }
   ...
   public synchronized void setString(int offset, String val) {
      contents.position(offset);
      if (val == null){
    	  System.err.println(" String should not be null");
    	  System.exit(1);
      }
      String modifiedVal= val + '\0';
      for (int i= 0 ; i< modifiedVal.length();i++){ // putChar()--->Writes two bytes containing the given char value,
    	                                             // in the current byte order,
    	                                            // into this buffer at the current position,
    	                                            // and then increments the position by two.
    	  contents.putChar(modifiedVal.charAt(i));
      }
   }
}
