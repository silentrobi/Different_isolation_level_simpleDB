 ...
 */
public class ExerciseBufferMgr {
   private static final long MAX_TIME = 10000; // 10 seconds
   private ExerciseBasicBufferMgr bufferMgr;
    ...
    */
   public ExerciseBufferMgr(int numbuffers) {
      bufferMgr = new ExerciseBasicBufferMgr(numbuffers);
   }
    ...
    */
   public synchronized ExerciseBuffer pin(Block blk) {
      try {
         long timestamp = System.currentTimeMillis();
         ExerciseBuffer buff = bufferMgr.pin(blk);
    ...
    * @return the buffer pinned to that block
    */
   public synchronized ExerciseBuffer pinNew(String filename, PageFormatter fmtr) {
      try {
         long timestamp = System.currentTimeMillis();
         ExerciseBuffer buff = bufferMgr.pinNew(filename, fmtr);
         while (buff == null && !waitingTooLong(timestamp)) {
            wait(MAX_TIME);
    ...
    */
   public synchronized void unpin(ExerciseBuffer buff) {
