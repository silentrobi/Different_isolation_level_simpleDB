...
public class RecordFile {
    ...
    * will wind up at the last record of the file. 
    */
   /* added*/
   public void afterLast(){
	   moveTo(tx.size(filename)-1); //move to last  block
   ...
   public boolean previous(){
	   ...
	   while(true){
		   ...
		   if (atFirstBlock())
			   return false;
		   moveTo(currentblknum-1); // going reverse order of blocks
		   rp.goToLastSlot(); // reading from last slot each time
    ...
    */
   public void insert() { 
      while (!rp.insert()) { // check each block to find empty slot in the file
         if (atLastBlock())
            appendBlock();
         moveTo(currentblknum + 1);
      }
