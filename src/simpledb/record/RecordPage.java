...
public class RecordPage {
   public static final int EMPTY = 0, INUSE = 1; // constant
   ...
   public RecordPage(Block blk, TableInfo ti, Transaction tx) {
      ...
      this.ti = ti;
      this.tx = tx;
      slotsize = ti.recordLength() + INT_SIZE;  // record size= record length + 4 instead of  record size= record length + 1 
      					 						//because simpleDB doesn't support one byte
...
/**
   ...
   public boolean insert() {
      currentslot = -1;
      boolean found = searchFor(EMPTY);// if no empty slot  found in the block then return false 
      // currentPosition change here other than -1
      if (found) {
         int position = currentpos();
         tx.setInt(blk, position, INUSE); // setting INUSE bit which takes 4 byte
      }
      return found;
   }
    ...
    */
   public void moveToId(int id) { // for indexing
      currentslot = id;
   }
    ...
    */
   public int currentId()  { // return slot id (index)
      return currentslot;
   }
   ...
   private int currentpos() { // indicates each slot's offset
   ...
   private boolean searchFor(int flag) { // search for empty slot if flag= EMPTY 
	   									 //or search for in use slot if flag= INUSE  
      currentslot++;
      
      while (isValidSlot()) {
         int position = currentpos();
         
         if (tx.getInt(blk, position) == flag)// find next in use record
            return true; 
         currentslot++;
      }
      return false;
   ...
   /** adding previous method to read backward
    * move previous record in the block
    * return false if no record found
    * */
   /* added*/
   public boolean previous(){
	   
	  return searchFromBackward(INUSE); // search backward for in use record
	  
   }
   
   
   /* added*/
   private boolean searchFromBackward(int flag){
        
    	   currentslot--;
    	   while(isValidSlotBackward()){ 
    		   int position= currentpos();
		   
    		   if(tx.getInt(blk, position)== flag) // find previous in use record
    			   return true;
    		   currentslot--; // accessing slot index backward
    	   }
   ...
   /* added*/
   private boolean isValidSlotBackward(){ // check slot index overflow
	   return currentpos() >= 0; 
   }
   
  /* added*/
   public void goToLastSlot(){
	  currentslot= (int)Math.floor(BLOCK_SIZE / slotsize);
